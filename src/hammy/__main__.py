...
def organize_pics(filenames: list[Path], resize: int | None = None) -> list[Path]:
	...
	for index, pic in enumerate(pics):
		...
		while pics[index].stat().st_size > 7600000:
			logging.warning(
				f'Image size is too big! Current Size: {pics[index].stat().st_size}'
			)
...
def get_progress(process_type: str) -> tuple[Progress, Progress]:
	description_progress = Progress(
		TextColumn(f'{process_type} ' + 'file: {task.fields[extra]}'),
		auto_refresh=False,
	)
...
def main() -> None:
		...
		with Live(group0):
			for u in urls:
				...
				if group0:
					...
					description_progress0.update(
						task1, advance=1, refresh=True, extra=u
					)
	...
	if sys.stdout.isatty():
		...
		task4 = description_progress1.add_task(
			'Task 4', total=total, extra=fname
		)  # put in a func?
	...
	with Live(group1):
		for pic in pics:
			...
			if group1:
				...
				description_progress1.update(
					task4, advance=1, refresh=True, extra=fname
				)
